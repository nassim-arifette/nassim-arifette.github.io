...
date: "2023-07-02"
description: "Intégration d’une projection sur le cône métrique dans Phyloformer pour garantir la métricité des distances phylogénétiques."
tags: ["Phylogenetics","Transformer","Phyloformer","Metric Projection","Convex Optimization","Triangle Inequality","Deep Learning"]
links: { pdf: "/bachelor_report_metric_projection_phyloformer.pdf" }
---
...
1. **Prédiction** :  $D_{hat} = f_{\theta}(X)$, matrice symétrique, diag zéro  
2. **Projection** : $D_{tilde} = Proj_{Mcone}(D_{hat})$ via un algorithme de proximité métrique $\ell_2$ (variante Brickell et al.)  
3. **Calcul de perte** : `L = ||D_tilde - D_star||_1`  
